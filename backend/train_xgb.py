# -*- coding: utf-8 -*-
# ============================================================
# XGBOOST  TIME SERIES PD ESTIMATION + FUTURE PD
# (Optuna | Expanding Window | Deployable Final Model)
# UPDATED: Overall Metrics & Detailed Feature Summary
# ============================================================

import os
import json
import optuna
import joblib
import numpy as np
import pandas as pd

from datetime import datetime
from xgboost import XGBClassifier
from sklearn.pipeline import Pipeline
from sklearn.preprocessing import StandardScaler
from sklearn.metrics import (
    roc_auc_score, accuracy_score,
    precision_score, recall_score, f1_score
)
from params_manager import load_best_params, save_best_params, load_best_metrics, save_best_metrics, compare_metrics

# ================= CONFIG =================

DATA_PATH = "Dataset_CD1_preprocessed_2.csv"
SAVE_DIR = "results/xgb"
MODEL_DIR = os.path.join(SAVE_DIR, "final_model")

MIN_TRAIN_YEARS = 1  # Số năm tối thiểu để bắt đầu train (2019 = 2018 + 1)
N_OPTUNA_TRIALS = 30  # Số lần thử nghiệm tìm tham số
RANDOM_STATE = 42

# Tạo thư mục lưu kết quả
os.makedirs(SAVE_DIR, exist_ok=True)
os.makedirs(MODEL_DIR, exist_ok=True)


# ================= HELPER FUNCTIONS =================

def get_dataset_path(filename="Dataset_CD1_preprocessed_1.csv"):
    """Tìm dataset ở vị trí đúng"""
    backend_dir = os.path.dirname(os.path.abspath(__file__))
    possible_paths = [
        os.path.join(backend_dir, filename),
        os.path.join(backend_dir, "uploads", filename),
        filename
    ]
    for path in possible_paths:
        if os.path.exists(path):
            return path
    raise FileNotFoundError(f"Dataset '{filename}' không tìm thấy. Tìm kiếm tại: {possible_paths}")


# ================= LOGGING =================

def log(msg):
    print(f"[{datetime.now().strftime('%H:%M:%S')}] {msg}")


# ================= METRICS UTILS =================

def eval_metrics(y_true, y_pred, y_prob):
    """Tính toán các chỉ số đánh giá mô hình"""
    return {
        "AUC": roc_auc_score(y_true, y_prob),
        "Accuracy": accuracy_score(y_true, y_pred),
        "Precision": precision_score(y_true, y_pred, zero_division=0),
        "Recall": recall_score(y_true, y_pred, zero_division=0),
        "F1": f1_score(y_true, y_pred, zero_division=0),
    }

# ================= FEATURE ANALYSIS =================

def grey_relational_analysis(X, y, feature_names, rho=0.5):
    """Grey Relational Analysis"""
    from sklearn.preprocessing import MinMaxScaler
    
    if isinstance(X, np.ndarray):
        X_df = pd.DataFrame(X, columns=feature_names)
    else:
        X_df = X
    
    scaler = MinMaxScaler()
    Xn = pd.DataFrame(scaler.fit_transform(X_df), columns=X_df.columns)
    
    if isinstance(y, np.ndarray):
        y_series = pd.Series(y)
    else:
        y_series = y.reset_index(drop=True)
    
    y0 = (y_series - y_series.min()) / (y_series.max() - y_series.min() + 1e-6)
    
    ref = y0.values
    diff = np.abs(Xn.values - ref.reshape(-1, 1))
    min_diff, max_diff = diff.min(), diff.max()
    
    xi = (min_diff + rho * max_diff) / (diff + rho * max_diff)
    gra_scores = xi.mean(axis=0)
    
    df_gra = pd.DataFrame({
        "feature": X_df.columns,
        "gra_score": gra_scores
    }).sort_values("gra_score", ascending=False)
    
    return df_gra

def run_influence_analysis_xgb(model, X_train, y_train, save_dir, feature_names):
    """Run complete influence analysis for XGBoost"""
    log("Running feature influence analysis...")
    
    # Get importance from XGBoost
    xgb_clf = model.named_steps['clf']
    importances = xgb_clf.feature_importances_
    
    df_imp = pd.DataFrame({
        "feature": feature_names,
        "importance": importances
    }).sort_values("importance", ascending=False)
    
    df_imp.to_csv(os.path.join(save_dir, "feature_importance_xgb.csv"), index=False)
    
    # GRA
    df_gra = grey_relational_analysis(X_train, y_train, feature_names)
    df_gra.to_csv(os.path.join(save_dir, "gra_scores.csv"), index=False)
    
    # Combine
    df = pd.merge(df_imp, df_gra, on="feature", how="inner")
    df["imp_rank"] = df["importance"].rank(ascending=False)
    df["gra_rank"] = df["gra_score"].rank(ascending=False)
    df["composite_score"] = 0.6 * (1 - df["imp_rank"] / len(df)) + 0.4 * (1 - df["gra_rank"] / len(df))
    df = df.sort_values("composite_score", ascending=False)
    
    df.to_csv(os.path.join(save_dir, "feature_influence_analysis_xgb.csv"), index=False)
    log("Feature analysis saved")

# ================= OPTUNA OBJECTIVE =================

def optuna_objective(trial, X, y, scale_pos_weight):
    """Hàm tối ưu hóa tham số cho XGBoost"""
    params = {
        "max_depth": trial.suggest_int("max_depth", 3, 8),
        "learning_rate": trial.suggest_float("learning_rate", 0.01, 0.3, log=True),
        "n_estimators": trial.suggest_int("n_estimators", 50, 300),
        "subsample": trial.suggest_float("subsample", 0.6, 1.0),
        "colsample_bytree": trial.suggest_float("colsample_bytree", 0.6, 1.0),
        "eval_metric": "logloss",
        "tree_method": "hist",  # Optimized histogram-based method
        "random_state": RANDOM_STATE,
        "scale_pos_weight": scale_pos_weight,
        "verbosity": 0
    }

    model = Pipeline([
        ("scaler", StandardScaler()),
        ("clf", XGBClassifier(**params))
    ])

    model.fit(X, y)
    y_prob = model.predict_proba(X)[:, 1]
    return roc_auc_score(y, y_prob)


# ================= PD SMOOTHING =================

def smooth_pd_by_cycle(df, window=3):
    """Làm trơn PD theo thời gian để tránh biến động quá gắt"""
    df = df.sort_values(["MaCP", "Year"])
    df["pd_smooth"] = (
        df.groupby("MaCP")["pd_default"]
        .transform(lambda x: x.rolling(window, center=True, min_periods=1).mean())
    )
    return df


# ================= 1. TIME-SERIES BACKTEST =================

def estimate_pd_time_series():
    log("Backtest: XGBoost Time-Series (Expanding Window)")

    df = pd.read_csv(get_dataset_path()).sort_values(["MaCP", "Year"])
    years = sorted(df["Year"].unique())

    # Các cột không dùng để train
    DROP_COLS = ["default_final", "STT", "MaCP", "Year", "split"]

    all_pd = []
    all_metrics = []
    all_params = []  # Track params from each year's Optuna tuning

    # --- VÒNG LẶP QUA CÁC NĂM (EXPANDING WINDOW) ---
    for i, year in enumerate(years):
        if i < MIN_TRAIN_YEARS:
            continue  # Bỏ qua các năm đầu chưa đủ dữ liệu (cần ít nhất MIN_TRAIN_YEARS năm để train)

        log(f"Training <= {year - 1} ... Predicting {year}")

        # Chia dữ liệu theo thời gian
        train_df = df[df["Year"] < year]
        test_df = df[df["Year"] == year]

        # Tách features và target
        X_train = train_df.drop(columns=DROP_COLS, errors="ignore").select_dtypes(include=[np.number])
        y_train = train_df["default_final"]
        X_test = test_df.drop(columns=DROP_COLS, errors="ignore").select_dtypes(include=[np.number])
        y_test = test_df["default_final"]

        # Tính toán class weight
        pos = np.sum(y_train == 1)
        neg = np.sum(y_train == 0)
        scale_pos_weight = neg / max(pos, 1)

        # Tối ưu Hyperparams bằng Optuna
        optuna.logging.set_verbosity(optuna.logging.WARNING)  # Giảm log rác
        study = optuna.create_study(direction="maximize")
        study.optimize(
            lambda t: optuna_objective(t, X_train, y_train, scale_pos_weight),
            n_trials=N_OPTUNA_TRIALS
        )
        best = study.best_params
        all_params.append(best)  # Collect params from each fold

        # Train model tốt nhất cho window hiện tại
        model = Pipeline([
            ("scaler", StandardScaler()),
            ("clf", XGBClassifier(
                **best,
                eval_metric="logloss",
                tree_method="hist",
                random_state=RANDOM_STATE,
                scale_pos_weight=scale_pos_weight,
                verbosity=0
            ))
        ])

        model.fit(X_train, y_train)

        # Predict
        y_prob = model.predict_proba(X_test)[:, 1]
        y_pred = (y_prob >= 0.5).astype(int)

        # Lưu metrics của năm hiện tại
        m = eval_metrics(y_test, y_pred, y_prob)
        m["Year"] = year
        all_metrics.append(m)

        # Lưu kết quả dự báo
        res = test_df.copy()
        res["pd_default"] = y_prob
        all_pd.append(res)

    # --- TỔNG HỢP KẾT QUẢ & TÍNH OVERALL METRICS ---
    pd_ts = pd.concat(all_pd)
    pd_ts = smooth_pd_by_cycle(pd_ts)

    # Tính Overall Metrics (Toàn bộ giai đoạn)
    y_true_total = pd_ts["default_final"]
    y_prob_total = pd_ts["pd_default"]
    y_pred_total = (y_prob_total >= 0.5).astype(int)

    total_metrics = eval_metrics(y_true_total, y_pred_total, y_prob_total)
    total_metrics["Year"] = "OVERALL"  # Đánh dấu dòng tổng

    # Gộp vào bảng metrics
    all_metrics.append(total_metrics)
    metrics_df = pd.DataFrame(all_metrics)

    # Lưu file
    pd_ts.to_csv(os.path.join(SAVE_DIR, "pd_time_series.csv"), index=False)
    metrics_df.to_csv(os.path.join(SAVE_DIR, "metrics_summary.csv"), index=False)
    
    # Lưu predictions_with_probability.csv cho frontend (company trends)
    predictions_df = pd_ts[["STT", "MaCP", "Year", "default_final"]].copy()
    predictions_df["Prediction"] = (pd_ts["pd_default"] > 0.5).astype(int)
    predictions_df["Probability"] = (pd_ts["pd_default"] * 100).round(2)
    predictions_df.to_csv(os.path.join(SAVE_DIR, "predictions_with_probability.csv"), index=False)
    
    # Lưu metrics_test.json cho frontend
    test_metrics = {
        "AUC": total_metrics["AUC"],
        "Accuracy": total_metrics["Accuracy"],
        "Precision": total_metrics["Precision"],
        "Recall": total_metrics["Recall"],
        "F1": total_metrics["F1"],
    }
    with open(os.path.join(SAVE_DIR, "metrics_test.json"), "w") as f:
        json.dump(test_metrics, f, indent=4)

    log(f" Hoàn tất Backtest. Kết quả lưu tại: {SAVE_DIR}")
    
    # Select best params based on AVERAGE AUC across all years (more stable)
    # This prevents overfitting to a single year's peak performance
    metrics_by_year = pd.DataFrame(all_metrics[:-1])  # Exclude OVERALL row
    avg_auc = metrics_by_year["AUC"].mean()
    best_year_idx = (metrics_by_year["AUC"] - avg_auc).abs().idxmin()  # Closest to average
    best_params = all_params[best_year_idx]
    log(f"📊 Selected params from year with AUC closest to average ({avg_auc:.4f})")
    
    return df, test_metrics, best_params


# ================= 2. FINAL MODEL FOR DEPLOYMENT =================

def train_and_save_final_model(df, best_params_from_cv, current_metrics):
    log(" Bắt đầu Train FINAL MODEL (Toàn bộ dữ liệu)")

    DROP_COLS = ["default_final", "STT", "MaCP", "Year", "split"]
    X = df.drop(columns=DROP_COLS, errors="ignore").select_dtypes(include=[np.number])
    y = df["default_final"]

    # Class weight
    pos = np.sum(y == 1)
    neg = np.sum(y == 0)
    scale_pos_weight = neg / max(pos, 1)

    # Try to load previous best params and metrics
    old_best_params = load_best_params(SAVE_DIR, "XGB")
    old_metrics = load_best_metrics(SAVE_DIR, "XGB")
    
    # Decide which params to use
    params_to_use = best_params_from_cv
    if old_best_params is not None:
        log(f"📊 Comparing: Old params vs New params from CV")
        log(f"   Old AUC: {old_metrics.get('AUC', 0):.4f}")
        log(f"   New AUC: {current_metrics.get('AUC', 0):.4f}")
        
        if compare_metrics(old_metrics, current_metrics):
            log(f"✅ New params are better!")
            params_to_use = best_params_from_cv
        else:
            log(f"📌 Old params are still better, using previous best")
            params_to_use = old_best_params
    else:
        log(f"📊 First run, using params from CV tuning")
        params_to_use = best_params_from_cv

    # Train Final Model
    final_model = Pipeline([
        ("scaler", StandardScaler()),
        ("clf", XGBClassifier(
            **params_to_use,
            eval_metric="logloss",
            tree_method="hist",
            random_state=RANDOM_STATE,
            scale_pos_weight=scale_pos_weight,
            verbosity=0
        ))
    ])

    final_model.fit(X, y)

    # --- TẠO BẢNG FEATURE SUMMARY & IMPORTANCE ---
    # Lấy Feature Importance từ XGBoost
    xgb_step = final_model.named_steps["clf"]
    importances = xgb_step.feature_importances_

    # Tính thống kê mô tả (Mean, Std, Min, Max...) của dữ liệu train
    feature_stats = X.describe().T
    feature_stats["importance"] = importances  # Thêm độ quan trọng
    feature_stats["dtype"] = X.dtypes  # Thêm kiểu dữ liệu

    # Sắp xếp theo độ quan trọng giảm dần
    feature_stats = feature_stats.sort_values("importance", ascending=False)
    feature_stats.reset_index(inplace=True)
    feature_stats.rename(columns={"index": "feature_name"}, inplace=True)

    # --- LƯU CÁC FILE MODEL ---
    # 1. Model object
    joblib.dump(final_model, os.path.join(MODEL_DIR, "pd_xgb_model.pkl"))
    # 3. Feature Schema (danh sách các feature đầu vào)
    json.dump({"features": X.columns.tolist()}, open(os.path.join(MODEL_DIR, "feature_schema.json"), "w"), indent=4)

    # Save the best params we decided to use
    save_best_params(params_to_use, SAVE_DIR)
    
    # Save metrics only if improved (or first time)
    if compare_metrics(old_metrics, current_metrics) or old_metrics is None:
        save_best_metrics(current_metrics, SAVE_DIR)
        log(f"✅ Saved new best metrics")
    else:
        log(f"📌 Kept previous metrics")
    
    # NOTE: Predictions are already saved from expanding window validation
    # Do NOT generate predictions using final model on all data, as it would violate time-series integrity
    # (model trained on future data would predict past years)
    # Frontend uses predictions_with_probability.csv from expanding window validation (2019-2024)
    
    # Generate predictions for 2025 using final model (synthetic future year)
    log("Generating predictions for 2025 (synthetic future year using final model)...")
    y_prob_2025 = final_model.predict_proba(X)[:, 1]
    predictions_2025 = df[["STT", "MaCP", "default_final"]].copy()
    predictions_2025["Year"] = 2025
    predictions_2025["Prediction"] = (y_prob_2025 > 0.5).astype(int)
    predictions_2025["Probability"] = (y_prob_2025 * 100).round(2)
    
    # Append 2025 predictions to CSV
    csv_path = os.path.join(SAVE_DIR, "predictions_with_probability.csv")
    try:
        if os.path.exists(csv_path):
            existing_df = pd.read_csv(csv_path)
            predictions_2025_reordered = predictions_2025[existing_df.columns]
            combined_df = pd.concat([existing_df, predictions_2025_reordered], ignore_index=True)
            combined_df.to_csv(csv_path, index=False)
            log(f"✅ Added 2025 predictions to CSV. Total years: {combined_df['Year'].unique()}")
        else:
            log(f"⚠️ CSV not found at {csv_path}")
    except Exception as e:
        log(f"❌ Error appending 2025 predictions: {e}")
    
    # Feature analysis - save to SAVE_DIR so frontend can read
    feature_names = X.columns.tolist()
    run_influence_analysis_xgb(final_model, X.values, y.values, SAVE_DIR, feature_names)

    log(f" Final Model đã sẵn sàng tại: {MODEL_DIR}")
    log("   (Bao gồm: pd_xgb_model.pkl, best_params.json, feature_influence_analysis_xgb.csv)")


# ================= WRAPPER FUNCTION FOR TRAIN_WRAPPER.PY =================

def train_xgb(save_dir="results/xgb", n_estimators=None, max_depth=None, learning_rate=None, 
              subsample=None, colsample_bytree=None, **kwargs):
    """Wrapper function để train_wrapper.py gọi (support hyperparameter overrides)"""
    global SAVE_DIR, MODEL_DIR
    SAVE_DIR = save_dir
    MODEL_DIR = os.path.join(save_dir, "final_model")
    
    os.makedirs(SAVE_DIR, exist_ok=True)
    os.makedirs(MODEL_DIR, exist_ok=True)
    
    # Chạy time-series training (now returns metrics and best_params too)
    df_full, current_metrics, best_params = estimate_pd_time_series()
    train_and_save_final_model(df_full, best_params, current_metrics)
    
    log(" Hoàn tất huấn luyện XGBoost")
    
    # Trả về metrics
    return current_metrics


# ================= MAIN EXECUTION =================

if __name__ == "__main__":
    # Bước 1: Chạy Backtest Time Series để đánh giá và sinh PD lịch sử
    df_full, metrics, best_params = estimate_pd_time_series()

    # Bước 2: Train Model cuối cùng trên toàn bộ dữ liệu để deploy
    train_and_save_final_model(df_full, best_params, metrics)

    log(" CHƯƠNG TRÌNH HOÀN TẤT THÀNH CÔNG!")
